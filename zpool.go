...
func poolGetConfig(name string, nv C.nvlist_ptr) (vdevs VDevTree, err error) {
	...
	if dtype = C.get_vdev_type(nv); dtype == nil {
		err = NewError(EUndefined, fmt.Sprintf("Failed to fetch %s", C.ZPOOL_CONFIG_TYPE))
		return
	}
	vdevs.Name = name
	...
	if vs = C.get_vdev_stats(nv); vs == nil {
		err = NewError(EUndefined, fmt.Sprintf("Failed to fetch %s", C.ZPOOL_CONFIG_VDEV_STATS))
		return
	}
	...
	for c := C.uint_t(0); children != nil && c < children.count; c++ {
		...
		vname := C.zpool_vdev_name(C.libzfs_get_handle(), nil, C.nvlist_array_at(children.first, c),
			C.B_TRUE)
		var vdev VDevTree
...
func poolGetSpares(name string, nv C.nvlist_ptr) (vdevs []VDevTree, err error) {
	...
	for c := C.uint_t(0); spares != nil && c < spares.count; c++ {
		vname := C.zpool_vdev_name(C.libzfs_get_handle(), nil, C.nvlist_array_at(spares.first, c),
			C.B_TRUE)
		var vdev VDevTree
		vdev, err = poolGetConfig(C.GoString(vname),
...
func poolGetL2Cache(name string, nv C.nvlist_ptr) (vdevs []VDevTree, err error) {
	...
	for c := C.uint_t(0); l2cache != nil && c < l2cache.count; c++ {
		vname := C.zpool_vdev_name(C.libzfs_get_handle(), nil, C.nvlist_array_at(l2cache.first, c),
			C.B_TRUE)
		var vdev VDevTree
		vdev, err = poolGetConfig(C.GoString(vname),
...
func PoolImportSearch(searchpaths []string) (epools []ExportedPool, err error) {
	...
	pools := C.go_zpool_search_import(C.libzfs_get_handle(), C.int(numofp), cpaths, C.B_FALSE)
	...
	for ; elem != nil; elem = C.nvlist_next_nvpair(pools, elem) {
		...
		if cname = C.get_zpool_name(config); cname == nil {
			err = NewError(EUndefined, fmt.Sprintf("Failed to fetch %s", C.ZPOOL_CONFIG_POOL_NAME))
			return
		}
		...
		if nvroot = C.get_zpool_vdev_tree(config); nvroot == nil {
			err = NewError(EUndefined, fmt.Sprintf("Failed to fetch %s", C.ZPOOL_CONFIG_VDEV_TREE))
			return
		}
...
func poolSearchImport(q string, searchpaths []string, guid bool) (name string,
	...
	if config == nil {
		err = NewError(EUndefined, fmt.Sprintf("No pool found %s", q))
		return
	}
	if guid {
	...
	if retcode := C.zpool_import_props(C.libzfs_get_handle(), config, cname,
		nil, C.ZFS_IMPORT_NORMAL|C.ZFS_IMPORT_ANY_HOST); retcode != 0 {
		err = NewError(EUndefined, fmt.Sprintf("Import pool properties failed: %s", LastError().Error()))
		return
	}
	return
}
...
func buildVDevTree(root *C.nvlist_t, rtype VDevType, vdevs, spares, l2cache []VDevTree,
	...
	for i, vdev := range vdevs {
		...
		if vcount < mindevs || vcount > maxdevs {
			err = NewError(EUndefined, fmt.Sprintf(
		...
		if grouping {
			if r := C.nvlist_alloc(&child, C.NV_UNIQUE_NAME, 0); r != 0 {
				err = NewError(EUndefined, "Failed to allocate vdev")
				return
			}
			csType := C.CString(string(vdev.Type))
...
func PoolCreate(name string, vdev VDevTree, features map[string]string,
	...
	csName := C.CString(name)
	defer C.free(unsafe.Pointer(csName))
	if r := C.zpool_create(C.libzfs_get_handle(), csName, nvroot,
		cprops, cfsprops); r != 0 {
		err = LastError()
...
func (pool *Pool) VDevTree() (vdevs VDevTree, err error) {
	...
	if config == nil {
		err = NewError(EUndefined, fmt.Sprintf("Failed zpool_get_config"))
		return
	}
